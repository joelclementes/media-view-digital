<nav x-data="{ open: false, catalogosOpen: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <!-- Catálogos Dropdown -->
                    @can('administrar_catalogos')
                        <div class="relative flex items-center" x-data="{ openCatalogosDesktop: false }">
                            <button type="button" x-on:click="openCatalogosDesktop = ! openCatalogosDesktop"
                                ...
                                request()->routeIs('cat-tipos-eleccion.*') || request()->routeIs('catalogos.sujetos') || request()->routeIs('cat-tamanos-publicacion.*') || request()->routeIs('catalogos.generos') || request()->routeIs('cat-violencia-temas.*') || request()->routeIs('cat-periodos.*')
                                ...
                                <svg class="ms-1 h-4 w-4 transition-transform duration-200"
                                    :class="{ 'rotate-180': openCatalogosDesktop }" xmlns="http://www.w3.org/2000/svg"
                                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                                </svg>
                            </button>

                            <div x-cloak x-show="openCatalogosDesktop" x-transition
                                class="absolute left-0 top-full z-50 mt-2 w-56 rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5">
                                <a href="{{ route('cat-tipos-eleccion.index') }}"
                                ...
                                <a href="{{ route('cat-violencia-temas.index') }}"
                                    class="block px-4 py-2 text-sm transition
                                    {{ request()->routeIs('cat-violencia-temas.*')
                                        ? 'bg-primary-50 text-primary-700 font-semibold'
                                        : 'text-gray-700 hover:bg-gray-100' }}">
                                    Temas de violencia
                                </a>
                                <a href="{{ route('cat-periodos.index') }}"
                                    class="block px-4 py-2 text-sm transition
                                    {{ request()->routeIs('cat-periodos.*')
                                        ? 'bg-primary-50 text-primary-700 font-semibold'
                                        : 'text-gray-700 hover:bg-gray-100' }}">
                    ...
                    @can('administrar_roles_permisos')
                        <x-nav-link href="{{ route('roles-permisos.index') }}" :active="request()->routeIs('roles-permisos.*')">
                            Roles y permisos
                        </x-nav-link>
                    @endcan
                </div>
    ...
    @can('administrar_catalogos')
        <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
            <div class="pt-2 pb-3 space-y-1">
                ...
                <div>
                    <button type="button" x-on:click="catalogosOpen = ! catalogosOpen"
                        ...
                        <svg class="h-4 w-4 transition-transform duration-200" :class="{ 'rotate-180': catalogosOpen }"
                    ...
                    <div x-cloak x-show="catalogosOpen" x-transition class="space-y-1 bg-gray-50 py-1">
                        <x-responsive-nav-link href="{{ route('cat-tipos-eleccion.index') }}" :active="request()->routeIs('cat-tipos-eleccion.*')"
                            class="ps-8">
                            Tipos de elección
                        </x-responsive-nav-link>
                        ...
                        <x-responsive-nav-link href="{{ route('cat-tamanos-publicacion.index') }}" :active="request()->routeIs('cat-tamanos-publicacion.*')"
                            class="ps-8">
                            Tamaños de publicación
                        </x-responsive-nav-link>
                        ...
                        <x-responsive-nav-link href="{{ route('cat-violencia-temas.index') }}" :active="request()->routeIs('cat-violencia-temas.*')"
                            class="ps-8">
                            Temas de violencia
                        </x-responsive-nav-link>
                        <x-responsive-nav-link href="{{ route('cat-periodos.index') }}" :active="request()->routeIs('cat-periodos.*')"
                            class="ps-8">
                            Periodos electorales
                        </x-responsive-nav-link>
                    </div>
            ...
            @can('administrar_roles_permisos')
                <x-responsive-nav-link href="{{ route('roles-permisos.index') }}" :active="request()->routeIs('roles-permisos.*')">
                    Roles y permisos
                </x-responsive-nav-link>
            @endcan
        </div>
