    ...
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Registro de televisión #{{ $registro->id }}
            </h2>

            <a href="{{ route('m-television-index') }}" class="inline-flex items-center rounded-lg bg-gray-700 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800">
                Volver
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow p-6 space-y-6">
                <div class="border-b pb-4">
                    ...
                    <p class="text-sm text-gray-500">Detalle completo del registro de televisión capturado.</p>
                </div>
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div><p class="text-xs font-semibold text-gray-500 uppercase">Tipo de señal</p><p class="text-gray-800 font-medium">{{ $registro->medio_tipo_senal ?: 'Sin tipo de señal' }}</p></div>
                        <div><p class="text-xs font-semibold text-gray-500 uppercase">Plaza</p><p class="text-gray-800 font-medium">{{ $registro->plaza_nombre ?? 'Sin plaza' }}</p></div>
                ...
                <div class="border-t pt-6">
                    <h3 class="text-lg font-bold text-gray-800 mb-4">Videos del registro</h3>
                    @if (count($archivos) > 0)
                        <div class="space-y-4">
                            @foreach ($archivos as $archivo)
                                <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
                                    ...
                                    <video controls preload="metadata" class="w-full rounded-lg">
                                        <source src="{{ asset('storage/' . $archivo) }}">
                    ...
                    @else
                        <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 text-gray-500">Este registro no tiene videos cargados.</div>
                    @endif
                </div>

                <div class="border-t pt-6 flex flex-col sm:flex-row gap-3 justify-end">
                    <a href="{{ route('m-television-testigo', $registro->id) }}" target="_blank" class="inline-flex justify-center rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700">
                        Ver testigo PDF
                    </a>
                    <a href="{{ route('m-television-index') }}" class="inline-flex justify-center rounded-lg bg-gray-700 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800">
                        Ir a televisión
                    </a>
                </div>
            </div>
